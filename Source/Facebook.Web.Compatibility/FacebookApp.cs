...
namespace Facebook
{
    ...
    public class FacebookApp : FacebookAppBase
    {
                ...
                if (this.session == null && this.Request != null)
                {
                    try
                    {
                        ...
                        if (this.session == null)
                        {
                            ...
                            {
                                this.session = ParseFacebookSession(this.AppSecret, this.Request.Params[this.SessionCookieName]);
                            }
                        }
                    }
                    catch
        ...
        private static void ResponseCallback(IAsyncResult asyncResult, FacebookAsyncCallback callback, object state)
        {
            ...
            try
            {
                ...
                using (Stream responseStream = response.GetResponseStream())
                {
                    ...
                    {
                        result = JsonSerializer.Current.DeserializeObject(reader.ReadToEnd());
                    }
                }
            }
            catch (FacebookApiException)
        ...
        internal static FacebookSession ParseFacebookSession(string appSecret, string value)
        {
            ...
            var signature = FacebookSession.GenerateSessionSignature(appSecret, dictionary);
